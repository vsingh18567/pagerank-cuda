    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        for (const auto &[node, in_nodes] : reverse_graph) {
            ...
            for (const auto &in_node : in_nodes) {
                ...
                // If out_degree = 0, that node is dangling and its rank is handled by dangling_sum
            }

            // Add dangling contribution: distributed evenly among all nodes
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Preamble: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(preamble - start).count() << "ms\n";
    std::cout << "Main Algorithm: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(main_algo - preamble).count() << "ms\n";
    std::cout << "Write Output: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(write_output - main_algo).count() << "ms\n";
    std::cout << "Total: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(write_output - start).count() << "ms\n";
