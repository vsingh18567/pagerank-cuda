...
Graph build_graph(const std::string &filepath) {
    ...
    // Initialize out_degree with zeros
    ...
    // Build adjacency structure for incoming edges (CSR format)
    // adj_incoming[i] will store all nodes that have an edge pointing to i.
    ...
    // Now build CSR representation from adj_incoming
    ...
    for (int i = 0; i < graph.num_nodes; i++) {
        // offset for node i
        graph.row_offsets.push_back(graph.row_offsets.back() + (int)adj_incoming[i].size());
...
std::vector<double> pagerank(const Graph &graph) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        for (int i = 0; i < graph.num_nodes; i++) {
            ...
            rank_new[i] = (1.0 - DAMPING_FACTOR) / graph.num_nodes + DAMPING_FACTOR * sum;
        }
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Preamble: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(preamble - start).count()
              << "ms\n";
    std::cout << "Main Algorithm: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(main_algo - preamble).count()
              << "ms\n";
    std::cout << "Write Output: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(write_output - main_algo).count()
              << "ms\n";
    std::cout << "Total: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(write_output - start).count()
              << "ms\n";
